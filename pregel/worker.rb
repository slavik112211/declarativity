...
class PregelWorker
  ...
  bloom :pregel_processing do
    ...
    queue_out_temp <= (vertices * queue_in)
      .outer(vertices.id => queue_in.vertex_id) do |vertex, queue_in|
        puts "hack" #this print statement is here, oddly enough, 
        # to prevent the lost of the first vertex. a bug in Bloom?
        # .outer() signifies a LEFT OUTER JOIN, so (vertices * queue_in).outer(vertices.id => queue_in.vertex_id)
        # should return all tuples present in "vertices".
        # For some reason, when "vertices" contains three vertices: 2,4,6,
        # "queue_out_temp" isn't supplied with the first tuple, and only gets 4,6
        # the println hack only works when "queue_out_temp" is populated in current Bloom timestep (<=, not <+)
        if(!control_pipe_input.empty? and control_pipe_input.any? {|control_message| control_message[0].command == "start" })
          messages = []
  ...
  bloom :debug_worker do
    ...
    stdio <~ queue_out { |vertex_message| [vertex_message.inspect] }
